    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', maxWidth: 430, margin: '0 auto', overflow: 'hidden' }}>
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {dbLoading ? (
          ...
          <>
            {activeTab === 'today' && (
              <HomeScreen entries={todayEntries} onAdd={addFood} onRemove={removeFood} onEdit={editFood} goal={settings.goal} macroGoals={settings.macroGoals} user={user} streak={(() => { let s = 0; for (let i = 0; i < 30; i++) { const k = new Date(Date.now() - i * 86400000).toISOString().split('T')[0]; if ((allEntries[k] || []).length > 0) s++; else break; } return s })()} />
            )}
