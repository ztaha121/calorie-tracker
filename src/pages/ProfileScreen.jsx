      ...
      {user ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[
            ...
            { label: 'Manage subscription', bg: 'var(--bg-card)', color: 'var(--text-muted)', border: 'var(--border)', action: () => { const email = user?.email || ''; const url = email ? `https://app.lemonsqueezy.com/my-orders?email=${encodeURIComponent(email)}` : 'https://app.lemonsqueezy.com/my-orders'; window.open(url, '_blank') } },
